  ...
  useEffect(() => {
    const fetchLoan = async () => {
      try {
        ...
        setError('Failed to fetch loan details: ' + (error.response?.data?.message || error.message));
      } finally {
        setLoading(false);
      }
    };
        ...
        <Card.Body className="p-4">
          <div className="d-flex justify-content-between align-items-start mb-4">
            ...
            <div className="text-end">
              ...
              <div className="fw-bold">{new Date(loan.fecha_creacion).toLocaleDateString()}</div>
            </div>
          </div>
